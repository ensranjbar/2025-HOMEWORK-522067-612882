...
public class LabirintoBuilderTest {
	...
	@Test
	public void testMonolocale_ConAttrezzo() {
		Labirinto monolocale = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale).aggiungiAttrezzo("spada",1)
				.addStanzaVincente(nomeStanzaIniziale).aggiungiAttrezzo("spadina", 3)
				.getLabirinto();
		assertEquals(nomeStanzaIniziale,monolocale.getStanzaIniziale().getNome());
		assertEquals(nomeStanzaIniziale,monolocale.getStanzaVincente().getNome());
	...
	@Test
	public void testMonolocale_ConAttrezzo_SingoloDuplicato() {
		Labirinto monolocale = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
				.aggiungiAttrezzo("spada",1)
				.aggiungiAttrezzo("spada",1)
				.getLabirinto();
		int size = monolocale.getStanzaIniziale().getAttrezzi().size();
		assertTrue(size==1);
		Set<Attrezzo> attesi = new HashSet<>();
		attesi.add(new Attrezzo("spada", 1));
		assertEquals(attesi,monolocale.getStanzaIniziale().getAttrezzi());
	}
	...
	public void testBilocale() {
		...
		Set<String> attesi = new HashSet<>();
		attesi.add("nord");
		Set<String> attesi2 = new HashSet<>();
		attesi2.add("sud");
		assertEquals(attesi,bilocale.getStanzaIniziale().getDirezioni());
		assertEquals(attesi2,bilocale.getStanzaVincente().getDirezioni());
	}
	
	@Test
	public void testTrilocale(){
		Labirinto trilocale = labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale).aggiungiAttrezzo("sedia", 1)
				.addStanza("biblioteca")
				.addAdiacenza(nomeStanzaIniziale, "biblioteca", "sud")
				.addAdiacenza("biblioteca", nomeStanzaIniziale, "nord")
				.aggiungiAttrezzo("libro antico", 5)
				.addStanzaVincente(nomeStanzaVincente)
	...
	@Test
	public void testTrilocaleCon_StanzaDuplicata() {
				labirintoBuilder
				.addStanzaIniziale(nomeStanzaIniziale)
...
???????????????????????????????????????????????????????????????????????????????	@Test
	public void testPiuDiQuattroAdiacenti() {
		...
		assertTrue(!maze.getStanzaIniziale().getMapStanzeAdiacenti().contains(test));
	...
	public void testAggiuntaAttrezziAStanze_Iniziale() {
		...
		Labirinto maze = this.labirintoBuilder
				.addStanzaIniziale(this.nomeStanzaIniziale)
				.aggiungiAttrezzo(nomeAttrezzo, peso)
				.getLabirinto();
	...
	public void testAggiuntaAttrezzoAStanze_MoltepliciAttrezziStessaStanza() {
		...
		.aggiungiAttrezzo(nomeAttrezzo1, peso1)
		.aggiungiAttrezzo(nomeAttrezzo2, peso2);
	...
	public void testLabirintoConStanzaBloccata_ConPassepartout() {
		this.labirintoBuilder
		.addStanzaIniziale(nomeStanzaIniziale)
		.addStanzaBloccata("stanza bloccata", "nord", "chiave").aggiungiAttrezzo("chiave", 1)
	...
	public void testLabirintoCompletoConTutteLeStanze() {
		...
		Labirinto labirintoCompleto = this.labirintoBuilder
				...
				.addStanzaVincente(nomeStanzaVincente)
				.addStanza("corridoio")
				.aggiungiAttrezzo("chiave", 1)
				.aggiungiAttrezzo("lanterna", 1)
